(function() {
  ...
  module.controller('PopupCtrl', function($scope, $window, $q, omegaTarget, profileIcons, profileOrder, dispNameFilter, getVirtualTarget) {
    ...
    $scope.applyProfile = function(profile) {
      // Ensure we're getting a valid profile object
      if (typeof profile === 'string') {
        profile = $scope.availableProfiles['+' + profile] || { name: profile };
      }
      ...
      chrome.runtime.sendMessage({
        ...
        if (response && response.success) {
          console.log('Profile switched successfully:', profile.name);
        } else {
          console.error('Failed to switch profile:', response);
        }
      });
    };
    $scope.tempRuleMenu = {
    ...
    $scope.clearData = function() {
      if (!confirm('Clear all browsing data and reload?\nThis will:\n\n' +
      ...
      chrome.runtime.sendMessage({
        ...
        if (response && response.success) {
          ...
          console.error('Failed to clear data:', response);
        }
      });
    };
  });
