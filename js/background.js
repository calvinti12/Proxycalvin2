(function() {
  ...
  const debugLog = (...args) => {
    console.log('[SwitchyOmega Debug]', ...args);
  };
  ...
  options.currentProfileChanged = function(reason) {
    debugLog('Profile change triggered:', reason);
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.method === 'applyProfile') {
      ...
      const profileName = request.args[0];
      ...
      chrome.browsingData.remove({
        ...
        // After clearing, create new window
        chrome.windows.create({
          ...
          chrome.windows.getAll((windows) => {
            ...
            // Apply the profile settings here
            // ... your existing profile application code ...

            // Show notification
            chrome.notifications.create({
              type: 'basic',
              iconUrl: '/img/icons/omega-64.png',
              title: 'SwitchyOmega',
              message: `Switched to ${profileName} profile and cleared browser data`
            });

            sendResponse({ success: true, profile: profileName });
          });
        });
      });
    ...
    if (request.method === 'clearAndReload') {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.method === 'clearAndReload') {
      console.log('TEST: Starting clear and reload process');
      
      // Simulate clearing data for testing
      if (TEST_MODE) {
        console.log('TEST: Would clear these data types:', {
          appcache: true,
          cache: true,
          cookies: true,
          downloads: true,
        ...
        // Simulate success
        setTimeout(() => {
          console.log('TEST: Data clearing simulation complete');
